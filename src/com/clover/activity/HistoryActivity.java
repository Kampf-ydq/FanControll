	...
	private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case ParamType.SUCCESS:
				...
				lv_his.setAdapter(new HistoryLVAdapter(HistoryActivity.this, R.layout.history_item, historyList));
				break;
	...
	private void initHistoryInfor(Object object){
    	...
    	try {
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				History history = new History(item.getString("number"),item.getString("statu"),item.getString("contactNumber"),
									  time);
